...
export default {
  ...
  computed: {
    channel_width_arr() {
      if (this.settings.hwmode === '11a') {
        ...
        return ['auto', 'HT20', 'HT40', 'HT40-', 'HT40+', 'HT80', 'HT80+', 'HT160']
  ...
  methods: {
    onHWModeChanged(value) {
      this.$emit('onHWModeChange', { label: `raido2_${value}`, name: 'VAP_5GHz_2' })
    },
    submit() {
      // TODO submit
      console.log('submit')
    ...
    getData() {
      group_get_config({
        ...
        res.raido2.paramsKey = 'radio2'
        res.raido2.group = this.$route.query.group
        this.settingsCopy = res.raido2
        this.settings = _.cloneDeep(res.raido2)
      })
    }
  }
}
