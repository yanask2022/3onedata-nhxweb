...
export default {
  ...
  data() {
    return {
      ...
      selectedRows: [],
      columns: [
        { label: 'Device name', prop: 'device_name' },
        { label: 'Group id', prop: 'group' },
        { label: 'Device id', prop: 'id' },
        { label: 'IP Address', prop: 'ip' },
        { label: 'Hardware Version', prop: 'hardware_version' },
    ...
    get_aplist({}).then(res => {
      if (res.aplist && Array.isArray(res.aplist)) {
        for (let i = 0; i < res.aplist.length; i++) {
          ...
          res.aplist[i].online = secondsToDhms(res.aplist[i].online) || '0 hours, 0 minutes, 0 seconds'
        }
