        ...
        <div class="field-row">
          ...
          <div class="field-value" style="display: flex; align-items: center">
            ...
            >
              ...
              <el-button v-else type="primary"  :disabled="!canUpgrade">Upload</el-button>
              <div slot="tip" class="el-upload__tip" v-if="!canUpgrade">{{cantTips}}</div>
            </el-upload>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      hw_name: 'NHX',
      alreadyFileName: '',
      cantTips: 'Only the hardware versions of the selected devices are the same to be upgraded in batches.'
    }
  },
  computed: {
    uploadData() {
  ...
  methods: {
    ...
    canBatchesUpgrade() {
      this.canUpgrade = this.parent.selectedRows.every(item => {
        return item.hardware_version === this.parent.selectedRows[0].hardware_version
      })
    }
  }
}
