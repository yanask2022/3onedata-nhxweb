...
export default {
  ...
  methods: {
    onSave() {
      ...
      const selectedRows = this.parent.selectedRows
      const params = {}
      params.device = ''
      // 左侧列表中当前选择的ap  device's id
      // params.device = selectedRow.id
      selectedRows.forEach(row => {
        params.device += row.id + ' '
      })
      params.device = params.device.substring(0, params.device.length - 1)
      // 左侧列表中当前选择的ap  device's  group
      params.group = this.selectedAp.group
